...
void Calibrate(std::string work_dir, int n_images, double raw_scale) {
  ...
  double scale = (n_cols * 13 - 1) / double(std::sqrt(2) * raw_scale);
  ...
  for (int x = 0; x < n_rows; x++) {
    for (int y = 0; y < n_cols; y++) {
      ...
      board_ids.emplace_back(x * n_cols + (n_cols - y - 1));
      //board_ids.emplace_back(x * n_cols + (n_cols - y));
      auto& current_vec = obj_points.back();
      current_vec.emplace_back(a / scale, b / scale, 0);
      current_vec.emplace_back((a + 10) / scale, b / scale, 0);
      current_vec.emplace_back((a + 10) / scale, (b + 10) / scale, 0);
      current_vec.emplace_back(a / scale, (b + 10) / scale, 0);
    }
  }
  ...
  for (int i = 0; i < n_images; i++) {
    ...
    std::cout << i << ":  "  << marker_corners.size() << " " << marker_ids.size() << std::endl;
